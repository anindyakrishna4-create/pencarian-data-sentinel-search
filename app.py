...
# --- Konfigurasi Halaman ---
st.set_page_config(
    page_title="Virtual Lab: Sentinel Search",
    layout="wide"
)

st.title("🛡️ Virtual Lab: Sentinel Search Interaktif (Matplotlib)")
st.markdown("### Visualisasi Algoritma Pencarian Efisien")
...
def plot_array(arr, target, current_index, found_index, max_val, status):
    ...
    # Inisialisasi warna: Biru (#4A86E8)
    colors = ['#4A86E8'] * n 
    
    # Tentukan Indeks Sentinel
    sentinel_idx = n - 1
...
# --- Visualisasi Utama ---
if st.button("Mulai Simulasi Sentinel Search"):
    
    found_index, history = sentinel_search(list(data_list), target_value)
    ...
    for step, state in enumerate(history):
        ...
        with status_placeholder.container():
            ...
            elif status in ('Selesai', 'Tidak Ditemukan'):
    ...
    st.info(f"Algoritma Sentinel Search selesai dalam **{len(history)-1}** langkah pengecekan.")
